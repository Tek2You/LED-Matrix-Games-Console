...
// uncomment to be able to use printf via uart
#define _USE_PRINTF_
#ifdef _USE_PRINTF_
#include <stdio.h>
#endif
...
/// \param buffer	pointer to data to be stored when received
/// \param nBytes	length of buffer
/// \param rxBytes	pointer to variable that stores the actual number of bytes received
/// \param rcvDone	character that marks the end of a stream of bytes to be received
/// \param flag		pointer to a variable that will indicate when the transfer is complete
...
//! Sets a callback function to be called when a byte is received
void uartSetRcvCallback(void (*callback)(char));

//! Sends a block of data via the uart using polling control.
/// \param buffer	null terminated pointer to data to be sent
void uartWriteString(uint8_t *buffer);

#ifdef _USE_PRINTF_
//! Sends a char from the file stream.  Required for using printf statements.
