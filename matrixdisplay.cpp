 ...
 *
 * SHOW DISPLAY
 * The display is multiplexed, that means that the leds not all have their own
 * connection to the controller, but each row and each column together connected
 * to the controller. Because of that, we cant set all leds at the same time(we
 * can, but only very all leds, not a text or something like we want. We only
...
const byte MatrixDisplay::letterWidth(char ch)
{
	if (ch < FIRST_LETTER || ch > LAST_LETTER)
		ch = '?';
	return pgm_read_word(PTN_LETTERS + ch - FIRST_LETTER + 1) -
	       pgm_read_word(PTN_LETTERS + ch - FIRST_LETTER);
}
